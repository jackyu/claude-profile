...
- Toast 顯示後端訊息（如「庫存不足」），不要通用的「伺服器錯誤」
- 401 自動觸發重新登入；網路錯誤給手動 retry 按鈕
- Sentry 回報時附完整 error response（status + message + errorCode）

## try..catch 規則

- API 呼叫、非同步操作必須用 try..catch 包起來
...
- 欄位級錯誤即時顯示（inline validation），送出失敗顯示具體訊息
- 成功操作給明確回饋（toast / redirect）
