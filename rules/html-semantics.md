# html-semantics.md — HTML 語意化與無障礙
...
- **禁止 `<div>` / `<span>` 當按鈕或連結**，該用 `<button>` / `<a>`
...
- 顏色對比度符合 WCAG AA（正文 4.5:1、大字 3:1）
- 不單靠顏色傳遞資訊，搭配圖示或文字
- 不要移除 `outline`（focus indicator），要改就用自訂 `focus-visible` 樣式
...
- 純觸發動作（無目的地 URL）用 `<button>`，不用空的 `<a href="#">`
