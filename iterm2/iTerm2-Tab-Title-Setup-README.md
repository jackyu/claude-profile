...
function set_iterm_tab_title() {
  ...
  if [[ -n "$branch" && -n "$worktree_root" ]]; then
    ...
    # Format: worktree:branch (e.g., mycompany-web:feature/auth)
...
| 情境 | Tab 標題 |
|------|----------|
| `cd ~/Projects/mycompany-web` (branch: main) | `mycompany-web:main` |
| `git checkout feature/auth` | `mycompany-web:feature/auth` |
| `cd ~` | `~` |
| `cd /tmp` | `tmp` |
